...
Four Codex findings, all about `CLAUDE.md` claiming something the tree does not say, all corrected. **Retention:** the pending list summarised ticket 15 as "nothing read older than 30 days is kept", which drops the starred exception and never says what the thirty days count from; the rule is now its own short list — Mirror, Horizon measured from when the article *became read*, and starred articles surviving both — matching `CONTEXT.md` and ticket 15. **Article id:** the file said "key articles by the decimal form" as if that were what the code does; today both GReader adapters produce and `Item.remote_id` stores the long `tag:google.com,2005:reader/item/<hex>` form and `ItemState` joins on it verbatim, so the file now says what is stored, says the decimal is ticket 09's recommendation for ticket 12 to act on, and warns that mixing the two forms breaks the read-state join. **The G7 command:** the table's inner Gradle line is shown as what G7 runs *after* its boot wait and AVD-name check, with the `adb -s emulator-5554 emu avd name` check a hand run must do first (or just run `scripts/check.sh`), plus the note that the test APKs stay installed because `gradle.properties` line 15 sets `android.injected.androidTest.leaveApksInstalledAfterRun=true`. **SDK lookup:** `scripts/android-sdk-path.sh` preferred the environment over `local.properties`, the reverse of AGP 8.10's `SdkLocator`; the script now reads `sdk.dir` from `local.properties` first, then `ANDROID_HOME`, then `ANDROID_SDK_ROOT`, its comment says why the two must agree, and `CLAUDE.md` describes that order. Verified: with no `local.properties` the script prints `/home/skynet/dev/android/sdk` and G0 and G7 both resolve the SDK; with a temporary `local.properties` holding a wrong `sdk.dir` the script reports that path instead, and `git status --ignored` confirms `local.properties` is ignored (`.gitignore` line 22). The temporary file was deleted and is not committed. Gate green G0 to G7.
