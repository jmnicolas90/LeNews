 ...
 * (see `apiModule`).
 */
class HttpClients(private val userAgent: String) {
    ...
    private fun buildClient(authInterceptor: AuthInterceptor?): OkHttpClient =
        OkHttpClient.Builder()
            ...
            .apply {
                // A network interceptor, so that it runs on every hop and a
                // redirect to another host is checked on its own.
                if (authInterceptor != null) addNetworkInterceptor(authInterceptor)
            }
            .build()
}
